(function () {
  ...
  const ZONE_DATA = {
    walls: {
      title: 'Стены',
      body: 'Оцениваем состав и толщину стены, её связь со смежными конструкциями, розетки, проходки и примыкания. Фактический результат может определяться самым слабым участком.',
      svg: () => `
    ...
    ceiling: {
      title: 'Потолок',
      body: 'Шум сверху передаётся через перекрытие и может распространяться дальше по стенам, колоннам и узлам. Исследуем прямой и фланговые маршруты до выбора потолочной системы.',
      svg: () => `
    ...
    floor: {
      title: 'Пол',
      body: 'Шаги, падения и работа оборудования возбуждают конструкции и распространяют вибрацию по перекрытию. Важны покрытие, стяжка, развязка и связи с соседними элементами.',
      svg: () => `
    ...
    joints: {
      title: 'Примыкания',
      body: 'Примыкания, швы, углы и проходки соединяют конструкции в общую систему. Ошибка в одном узле способна ограничить результат всей звукоизоляционной конструкции.',
      svg: () => `
    ...
    engineering: {
      ...
      body: 'Вентиляция, стояки, трубы, шахты и оборудование создают собственный шум и формируют обходные каналы между помещениями. Для них требуются отдельные решения.',
      svg: () => `
    ...
    equipment: {
      title: 'Оборудование',
      body: 'Техника и инженерные агрегаты создают шум и вибрацию, которые передаются по конструкциям и влияют на соседние помещения.',
      svg: () => `
        <text x="16" y="22" class="viz-label viz-label--title">ИСТОЧНИК · ОБОРУДОВАНИЕ</text>
        <rect x="72" y="72" width="88" height="96" class="viz-zone" rx="2"/>
        <text x="116" y="128" class="viz-label viz-label--active" text-anchor="middle">агрегат</text>
        ${CIAViz.roomBlock(180, 56, 100, 136, 'помещение')}
        <circle cx="116" cy="100" r="8" class="viz-node"/>
        ${CIAViz.impactRipples(116, 100)}
        <path d="M160 110 H180" class="viz-path" marker-end="url(#arrow)"/>
        <text x="200" y="100" class="viz-label viz-label--caption">вибрация</text>
        ${CIAViz.movingDot('M160 110 H180', '#2F7DFF', '1.5s')}
        ${CIAViz.pathLegend(16, 196)}
      `,
    },
    acoustics: {
      title: 'Внутренняя акустика',
      body: 'Реверберация, ранние отражения и распределение звуковой энергии влияют на речь, прослушивание и ощущение комфорта. Эта задача проектируется отдельно от звукоизоляции.',
      svg: () => `
